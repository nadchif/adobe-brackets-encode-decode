...
define(function(require, exports, module) {
  ...
  const encodeToHtmlEntities = require('convertors/htmlentities')
      .encodeToHtmlEntities;
  const decodeFromHtmlEntities = require('convertors/htmlentities')
  ...
  const encodeToPHPSerial = require('convertors/PHPSerialize')
      .encodeToPHPSerial;
  const encodeIntToPHPSerial = require('convertors/PHPSerialize')
      .encodeIntToPHPSerial;
  const encodeFloatToPHPSerial = require('convertors/PHPSerialize')
      .encodeFloatToPHPSerial;
  const decodeFromPHPSerial = require('convertors/PHPSerialize')
      .decodeFromPHPSerial;
  const encodeToMorseCode = require('convertors/morsecode').encodeToMorseCode;
  const decodeFromMorseCode = require('convertors/morsecode')
      .decodeFromMorseCode;
  const encodeUnixToUTCDate = require('convertors/unixToUTCDate')
      .encodeUnixToUTCDate;
  ...
  const ecdcMenu = Menus.getContextMenu(
      Menus.ContextMenuIds.EDITOR_MENU,
  ).addSubMenu('Encode/Decode Selection', 'NADCHIF_ECDC_MENU');

  const stageMenu = (id, categoryTitle, categoryTools) => {
    const stagedMenuEncoders = [];
    const stagedMenuDecoders = [];
    const subMenu = ecdcMenu.addSubMenu(
        categoryTitle,
        `NADCHIF_ECDC_MENU_${id}`,
        Menus.FIRST,
    );
    categoryTools.forEach((item, index) => {
      if (item.encoder) {
        CommandManager.register(
          item.encodeTitle ? item.encodeTitle : `String to ${item.title}`,
          `ecdc_enc_${id}${index}`,
          () => {
        ...
        stagedMenuEncoders.push({
          title: item.encodeTitle ?
            item.encodeTitle :
            `String to ${item.title}`,
          cmd: `ecdc_enc_${id}${index}`,
        });
      }
      if (item.decoder) {
        CommandManager.register(
          item.decodeTitle ? item.decodeTitle : `${item.title} to String`,
          `ecdc_dec_${id}${index}`,
          () => {
        ...
        stagedMenuDecoders.push({
          title: item.decodeTitle ?
            item.decodeTitle :
            `${item.title} to String`,
          cmd: `ecdc_dec_${id}${index}`,
        });
      }
    });
  ...
  stageMenu(
      'php',
      'PHP',
      ENCODERS_DECODERS.filter((item) => item.category == 'php'),
  );
  stageMenu(
      'crypto',
      'Crypto',
      ENCODERS_DECODERS.filter((item) => item.category == 'crypto'),
  );
});
